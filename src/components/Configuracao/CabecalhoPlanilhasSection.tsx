...
export default function CabecalhoPlanilhasSection({ value, onChange, onSave, saving }: Props) {
  ...
  const handleNavigateToHeaderForm = () => {
    navigate("/configuracao/cabecalho-planilha");
  };
    ...
    (async () => {
      try {
        ...
        const headersEntries = await Promise.all(
          codes.map(async (code) => {
            const headerSnap = await get(ref(db, `/units/${code}/settings/spreadsheetHeader`));
  ...
  const handleDeleteHeader = async (unitCode: string) => {
    ...
    try {
      if (header && header.imagemUrl) {
        const url: string = header.imagemUrl as string;
        if (url.startsWith("http") || url.startsWith("gs://")) {
          const sref = storageRef(storage, url);
          try { await deleteObject(sref); } catch {}
        }
      }
      await set(ref(db, `/units/${unitCode}/settings/spreadsheetHeader`), null);
  ...
  return (
    <section className="bg-white border rounded-lg p-6 shadow-card">
      <h2 className="text-lg font-semibold text-gray-text mb-2">Cabeçalho — Planilhas</h2>
      <p className="text-sm text-gray-light mb-4">Gerencie o cabeçalho exibido nas planilhas com imagem e autoridades.</p>
              ...
              return (
                <li key={u.code} className="py-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-gray-light">{h ? (h.titulo || "Cabeçalho cadastrado") : "Sem cabeçalho"}</div>
                  </div>
